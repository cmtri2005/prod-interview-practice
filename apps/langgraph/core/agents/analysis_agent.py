...
class AnaLysisAgent(BaseAgent):
    def __init__(self, agent_name, llm, tools):
        ...
        feild_des = "\n".join(
            ...
            for name, field in Topic.model_fields.items()
        )
        self.prompt = ChatPromptTemplate(
            [
                (
                    "system",
                    f"""You are an instructional designer. 
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        if idx_module is None:
            self.logger.info(
                "Graph signaled awaiting_input -> breaking astream to return to client"
            )
        ...
        if state.get("exit_graph") is not None and state.get("exit_graph"):
            ...
            return {"messages": messages, "exit_graph": None, "next_agent": "END"}
        ...
        try:
            ...
            self.tools["save_raw_output"](
                f"Module_selected{idx}_output.json", module.model_dump_json(indent=2)
            )
            ...
            try:
                ...
                if attempt < self.MAX_RETRIES:
                    ...
                    messages.append(
                        (
                            "ai",
                            f"Failed to parse Analysis Module after {self.MAX_RETRIES} attempts. Error: {e}",
                        )
                    )
                    break

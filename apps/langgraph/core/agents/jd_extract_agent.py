...
class JDExtractAgent(BaseAgent):
    def __init__(self, agent_name, llm, tools):
        ...
        feild_des = "\n".join(f"- {name}: {field.description}" for name, field in JD.model_fields.items())
        self.prompt = ChatPromptTemplate([
            ("system", f"""You are a specialized Job Description (JD) extraction assistant.
    ...
    def extract_jd(self, state: AgentState) -> AgentState:
        try:
            ...
            raw = chain.invoke({"jd_content": jd_content, "format_instructions": self.parser.get_format_instructions()})
            ...
            self.tools["save_raw_output"]("JD_output.json", json.dumps(jd_dict, indent=2, ensure_ascii=False))
